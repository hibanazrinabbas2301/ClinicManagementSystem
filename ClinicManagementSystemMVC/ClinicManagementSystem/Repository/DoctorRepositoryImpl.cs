    ...
    public class DoctorRepositoryImpl : IDoctorRepository
    {
        ...
        public void AddLabPrescription(LabPrescription model)
        {
            ...
            cmd.Parameters.AddWithValue("@TestId",
                    model.TestId == null ? DBNull.Value : model.TestId);
        ...
        public List<MedicinePrescription> GetPatientMedicineHistory(int patientId)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_PatientMedicineHistory", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@PatientId", patientId);
        ...
        public List<LabPrescription> GetPatientLabHistory(int patientId)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_PatientLabHistory", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@PatientId", patientId);
            ...
            while (dr.Read())
            {
                list.Add(new LabPrescription()
                {
                    TestName = dr["TestName"].ToString(),
                    Status = dr["Status"].ToString(),
                    PrescribedDate = Convert.ToDateTime(dr["PrescribedDate"]),
                    ResultText = dr["ResultText"] == DBNull.Value ? null : dr["ResultText"].ToString(),
                    ResultDate = dr["ResultDate"] == DBNull.Value ? null : Convert.ToDateTime(dr["ResultDate"])
                });
            }
